...
- (id)initWithDictionary:(NSDictionary *)adNetConfigDict
  ...
  if (self != nil) {
    ...
    if (cred == nil) {
	  ...
	  // [tim] [add for Geo IP feature]
	  
	  if (networkType == 50)	// Geo IP 
	  {
		  NSString *str = [credentials objectForKey:kAdWhirlPubIdKey];
		  
		  // string format should be CN, key 
		  
		  NSArray *ary = [str componentsSeparatedByString:@","];
		  
		  NSString *strNation = (NSString *)[ary objectAtIndex:0];
		  
		  NSString *strKey = (NSString *)[ary objectAtIndex:1];
		  ...
		  if ([strNation compare:@"TW"] == NSOrderedSame)
		  {
			  networkType = AdWhirlAdNetworkTypeAdOnTW;
			  
			  credentials = [[NSDictionary alloc] initWithObjectsAndKeys:
							 [NSString stringWithString:strKey], kAdWhirlPubIdKey,
							 nil];
		  }
		  else  if ([strNation compare:@"CN"] == NSOrderedSame)
		  {
			  networkType = AdWhirlAdNetworkTypeAdOnCN;
			  ...
			  NSLog(@"error, unidentified Geo key");
		  }
